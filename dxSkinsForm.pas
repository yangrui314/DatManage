...
begin
  inherited Update;
  if (Handle <> 0) and HasRegion and not IsSkinUsed and (Form <> nil) then
  begin
    HasRegion := False;
...
begin
  ...
  if TrackedScrollBar <> AScrollBar then
    Windows.GetScrollBarInfo(Handle,
      Integer(ScrollBarOBJIDs[AScrollBar = saeVertScroll]), Result)
  else
  begin
...
var
  ASysMenu: THandle;
begin
  if IsWin2K or NativeBorderWidth then
  begin
    GetSystemMenu(Handle, True); // Win2k redrawing bug
    if IsDialog and HasBorder and not IsMDIChild then
    begin
...
function TdxSkinFormNonClientAreaInfo.GetIsMiniMize: Boolean;
begin
  Result := WindowInfo.dwStyle and WS_MINIMIZE = WS_MINIMIZE;
end;
...
begin
  if ViewInfo.HasMenu then
    FillRect(Canvas.Handle,
      cxRectSetBottom(ViewInfo.MenuBounds, ViewInfo.MenuBounds.Bottom, 1),
...
procedure TdxSkinFormPainter.InternalDrawBorder(const R: TRect;
  ...
  begin
    FBordersCache[ASide].CheckCacheState(
      PainterInfo.FormFrames[not ViewInfo.ToolWindow, ASide], R, FrameStates[Active]);
    FBordersCache[ASide].Draw(DC, R);
  end;
...
begin
  if not AFillBackground then
    DrawBorderElement(Canvas.Handle, R)
  else
  begin
    R1 := R;
...
begin
  ...
  if Result then
    ...
    Result := (TMessage(AMessage).WParam = TMessage(AMessage).LParam) and
      inherited WMEraseBk(AMessage);
end;
...
begin
  ...
  begin
    if TdxSkinFormController.IsMDIClientWindow(AHandle) or
      IsWindowUnicode(AHandle) // todo: try to detect TntControls
    then
