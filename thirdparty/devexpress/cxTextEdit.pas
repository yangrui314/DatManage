...
const
  WM_REDRAWMISSPELLS: Cardinal = WM_DX + 100;
...
procedure PrepareTextRows(ACanvas: TCanvas; var TextOutData: TcxTextOutData;
  ...
  var
    ATextLength: Integer;
    wStr: WideString; // Added by Omni
  begin
    TextOutData.RowCount := 0;
    cxResetTextRows(TextOutData.TextRows);
    wStr := AText;  // Added by Omni
    //ATextLength := StrLen(AText);
    ATextLength := Length(wStr);  // Added by Omni
...
function TcxCustomTextEditViewInfo.NeedShowHint(ACanvas: TcxCanvas;
  ...
  begin
    ...
    Result := not cxMakeTextRows(ACanvas.Canvas, PChar(Text), Length(Text),
    ...
    begin
      ...
      begin
        ...
        ARowText := TrimRight(Copy(ATextRow.Text, 1, ATextRow.TextLength));
        if ARowText = '' then
          ASize := cxSize(0, ACanvas.TextHeight('Zg'))
        else
          GetTextExtentPoint32(ACanvas.Handle, PChar(ARowText), Length(ARowText), ASize);
...
begin
  Result := inherited IsChildWindow(AWnd) or
    (Assigned(dxSpellCheckerEditHelper) and dxSpellCheckerEditHelper.IsSpellCheckerDialogControl(AWnd));
end;
...
procedure TcxCustomTextEdit.DoDrawMisspelleds;
begin
  if Assigned(dxSpellCheckerEditHelper) then
    dxSpellCheckerEditHelper.DoDrawMisspelleds(InnerControl);
end;

procedure TcxCustomTextEdit.DoSelectionChanged;
begin
  if Assigned(dxSpellCheckerEditHelper) then
    dxSpellCheckerEditHelper.SelectionChanged(InnerControl);
end;
...
begin
  ...
  if Assigned(dxSpellCheckerEditHelper) then
    dxSpellCheckerEditHelper.LayoutChanged(InnerControl);
end;

procedure TcxCustomTextEdit.DoTextChanged;
begin
  if Assigned(dxSpellCheckerEditHelper) then
    dxSpellCheckerEditHelper.TextChanged(InnerControl);
end;
...
begin
  case Message.Msg of
    ...
    CN_CTLCOLOREDIT:
      ...
      if Message.Msg = WM_REDRAWMISSPELLS then
        DoDrawMisspelleds;
  end;
end;
...
begin
  if InnerControl.HandleAllocated then
    PostMessage(InnerControl.Handle, WM_REDRAWMISSPELLS, 0, 0);
end;
...
function TcxTextEdit.IsCheckAsYouTypeSupports: Boolean;
begin
...
initialization
  WM_REDRAWMISSPELLS := RegisterWindowMessage('WM_REDRAWMISSPELLS');
